...
def train(train_loader, model, criterion, optimizer, epoch, device, print_freq=25):
    ...
    for i, (input_id, mask, token_id, target) in enumerate(train_loader):
        ...
        # Forward pass
        logits = model(input_id, mask, token_id)
        ...
        losses.update(loss.item(), input_id.size(0))
...
def eval(val_loader, model, criterion, device):
    ...
    with torch.no_grad():
        for i, (input_id, mask, token_id, target) in enumerate(val_loader):
...
if __name__ == "__main__":
    ...
    commandLineParser.add_argument('TRAIN_PROMPTS', type=str, help='training data prompts mlf')
    ...
    train_prompts_mlf = args.TRAIN_PROMPTS
